...
import * as widgetList from '../../widgets/list.es6'; // 组件列表
...
class WidgetsClass extends BaseClass {
     ...
     */
    newUserWidgetID(widget){
        let d = +new Date();
        let id = (widget && widget.id) ? widget.id * 1e4 : 1e7;
        return id + +d.toString().slice(4, 11);
    }
    ...
    stringifyUserWidget(widget){
        ...
        delete props.layoutUiId;
        delete props.uiDetailId;
        delete props.lightDeviceId;
        delete props.lightDevices;
