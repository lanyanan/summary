...
(function(global, factory) {
    ...
    Calendar.prototype = $.extend({}, Widget.prototype, {
        ...
        _onToday: function() {
            ...
            //触发"今天"监听函数
            this.fire('today', this.getCurrDateObj('today'));
        },
        ...
        _goPreOrNextMon: function(type, date) {
            ...
            this.fire('changeMonBefore', this.getCurrDateObj(type), type);
        ...
        //改变月份执行
        _changeMonth: function(year, month, date, type,tag) {
            //调整年月日
            //this._adjustTime(year, month, date);

            //重新生成日历
            this.reDrawCalendar(year, month, date);
            var newdt=this.getCurrDateObj()+
        ...
        tplCalendarWrap: function() {
            ...
            html = '<div class="calendar">' + '<div class="cal-header">'+'<span class="pre-mon">＜</span>' + '<span class="title-wrap">' + '<span class="year">' + this.get('year') + '</span> 年 ' + '<span class="month">' + this.get('month') + '</span> 月 ' + '<span class="head-text">' + this.get('headText') + '</span>' + '</span>' + '<span class="next-mon">＞</span>' + '</div>';

            if (this.cfg.showToday) {
                html += '<span class="today">今天</span>';
            }
 
            html +=  '<div class="days">' + '<span>日</span><span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span>六</span>' + '</div>' + '</div>';
        ...
        getCurrDateObj: function(type) {
            ...
            return {
                ...
                date: this.get('date'),
                type:type,
                tag:function (tagDates) {
                    console.log('tagDates'+tagDates);
                    _this.set('tagDates',tagDates);
                    _this.reDrawCalendar(_this.get('year'), _this.get('month'), _this.get('date'));
                } 
            };
        },
