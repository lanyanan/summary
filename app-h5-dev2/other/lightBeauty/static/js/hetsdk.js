...
({
	...
	/***/
		...
		__webpack_require__(13); // iOS最新框架补丁
	...
	/***/
		...
		window.hetsdk = function() {
			'use strict';

			var appInterfaceNS = 'bindJavaScriptSDK'; // app注入接口的命名空间
			var webInterfaceNS = 'webInterfaceSDK'; // web暴露给app调用接口的命名空间
			function HET() {
				// 默认配置
				var settings = {
				...
				function getCurrentDeviceSn() {
					...
					sn = sn.length > 32 ? sn.substring(0, 32) : sn;
					return sn;
					// return window.AppJsBridge.service.deviceService.getCurrentDeviceSn().substring(0, 32);
				}
				...
				function getToken(sucCallback, errCallback) {
					...
					var errFun = function errFun(data) {
						return typeof errCallback === 'function' && errCallback('获取token失败,APP代理token请求失败');
					};
					var sucCallbackId = register(sucFun); // 登记成功时的回调
					var errCallbackId = register(errFun); // 登记异常时的回调
					window.AppJsBridge.doAction({
						"sn": "25a11ee2-ecba-4c06-8b8f-36f854f7c71b",
						"deviceClass": "generalSwitch",
						"action": 'getAccessToken',
						"success": sucCallbackId,
						"error": errCallbackId
					});
				}
