...
/***/ function(module, exports, __webpack_require__) {
	...
	var _TipView = __webpack_require__(101);

	var _SetingPage = __webpack_require__(102);

	var _SetingPage2 = _interopRequireDefault(_SetingPage);

	var _AromaSegView = __webpack_require__(98);

	var _ColorSelectView = __webpack_require__(103);

	var _ColorRingView = __webpack_require__(104);

	var _YYSlide = __webpack_require__(107);

	var _Slider = __webpack_require__(108);

	var _AromaMistAndTimeView = __webpack_require__(99);

	var _TimeShowView = __webpack_require__(109);

	var _PickerView = __webpack_require__(100);

	var _CloseView = __webpack_require__(110);

	var _PlanSettingMain = __webpack_require__(111);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	/*import ReactCSSTransitionGroup from 'react-addons-css-transition-group';*/

	__webpack_require__(115); // import {Funs} from '../../../common/src/fun.es6';
	...
	var App = function (_BaseComponent) {
	    ...
	    function App(props) {
	        ...
	        _this.state = {
	            tipState: _TipView.TipState.TipStateMistTip,
	            modeState: _AromaSegView.ModeState.ModeState3,
	            color: '#ff7b7c',
	            isShow: false,
	            brightness: Number(50),
	            mist: _AromaMistAndTimeView.MistState.MistStateHalf,
	            timeClose: 30,
	        ...
	        //
	        _this.isSelectingColor = false;
	        _this.useSetTime = false;
	        return _this;
	    }

	    (0, _createClass3.default)(App, [{
	        key: 'handleDisplayData',
	        value: function handleDisplayData(dic) {
	            var updateObj = {};
	            console.log("handleDisplayData");
	            var warn = parseInt(dic["warring1"]);
	            //提示控制
	            if (warn === 1) {
	                // this.setState({tipState:TipState.TipStateLackWater});
	                $.extend(updateObj, { tipState: _TipView.TipState.TipStateLackWater });
	            } else if (this.state["tipState"] === _TipView.TipState.TipStateLackWater) {
	                // this.setState({tipState:TipState.TipStateClose});
	                $.extend(updateObj, { tipState: _TipView.TipState.TipStateClose });
	            }
	            //mode
	            var mode = parseInt(dic["mode"]);
	            $.extend(updateObj, { modeState: mode });

	            var brightness = parseInt(dic["brightness"]);
	            if (mode === _AromaSegView.ModeState.ModeState3) {
	                $.extend(updateObj, { brightness: brightness });
	                //颜色控制
	                if (this.isSelectingColor == false) {
	                    var red = parseInt(dic["red"]);
	                    var green = parseInt(dic["green"]);
	                    var blue = parseInt(dic["blue"]);
	                    var colorStr = "#" + red.toString(16) + green.toString(16) + blue.toString(16);
	                    $.extend(updateObj, { color: colorStr });
	                }
	            }

	            var mist = parseInt(dic["mist"]);
	            $.extend(updateObj, { mist: mist });

	            if (this.userSetTime) {
	                var mins = parseInt(dic["timeCloseH"]) * 60 + parseInt(dic["timeCloseM"]);
	                mins = isNaN(mins) ? 0 : mins;
	                $.extend(updateObj, { timeClose: mins });
	            } else {
	                var _mins = parseInt(dic["remainingTimeH"]) * 60 + parseInt(dic["remainingTimeM"]);
	                _mins = isNaN(_mins) ? 0 : _mins;
	                $.extend(updateObj, { timeClose: _mins });
	            }

	            var presetHour = $.parseIntInvalidToZero(dic["presetOpenH"]);
	            var presetMin = $.parseIntInvalidToZero(dic["presetOpenM"]);
	            var orderBrightness = $.parseIntInvalidToZero(dic["orderBrightness"]);
	            var orderMist = $.parseIntInvalidToZero(dic["orderMist"]);

	            if (presetHour * 60 + presetMin > 0 || orderBrightness > 0 || orderMist > 0) {
	                //有预约开机设置
	                var colorR = dic["orderRed"].toTwoHex();
	                var colorG = dic["orderGreen"].toTwoHex();
	                var colorB = dic["orderBlue"].toTwoHex();
	                var orderColorStr = "#" + colorR + colorG + colorB;
	                if (orderColorStr === "#000000") {
	                    orderColorStr = "#ffffff";
	                }
	                var mistArr = ["关闭", "大雾", "小雾", "睡眠"];
	            ...
	            //开关机
	            if (mode === _AromaSegView.ModeState.ModeState3 && brightness === 0 && mist === _AromaMistAndTimeView.MistState.MistStateClose || //单色模式下的关机
	            mode === _AromaSegView.ModeState.ModeState0 && mist === _AromaMistAndTimeView.MistState.MistStateClose) {
	                //轮播模式0的关机
	                //  alert("close");
	        ...
	        value: function componentDidMount() {
	            het.setTitle('香薰机');
	            this.removeListen = _Store.Store.listen(this.handleDisplayData.bind(this));
	            // Actions.getDisplayData();
	            if (this.isEmptyObject(_Store.Store.displayData) === false) {
	                this.handleDisplayData(_Store.Store.displayData);
	            }
	            var aromaCanvas = this.refs.aromaCanvas;
	            var factor = 2;
	            var context = aromaCanvas.getContext("2d");
	            context.beginPath();
	            ...
	            //初始化关机的view
	            var height = window.screen.height - parseInt($(".upView").css("height"));
	            height = height + "px";
	        ...
	        value: function switchHandle(e) {
	            this.state.switchState === 0 ? _Actions.Actions.changeSwitch(1) : _Actions.Actions.changeSwitch(0);
	        }
	        //改变定时关闭时间
	        ...
	        value: function render() {
	            ...
	            var colorRingViewIsShow = this.state.isShow;
	            var colorDom = void 0;
	            if (this.state.modeState === _AromaSegView.ModeState.ModeState3) {
	                ...
	                colorDom = React.createElement(
	                    ...
	                    React.createElement(
	                        'h2',
	                        { style: { fontSize: '16px', padding: '15px', paddingBottom: '0px', color: '#9693b2', backgroundColor: this.state.cocolor } },
	                        '\u989C\u8272'
	                    ),
	                    React.createElement(_ColorSelectView.ColorSelectView, { currentColor: this.state.color, changeColorHandle: this.changeColorHandle.bind(this) }),
	                    React.createElement(_Slider.Slider, { min: '0', max: '100', changeValue: this.changValueHandle.bind(this), value: this.state.brightness,
	                        showText: Boolean(true) })
	                );
	            }
	            var showOrderSet = this.state.switchState === 0 ? false : true;
	            return React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    React.createElement('a', { href: '#', className: 'switch', onTouchEnd: this.switchHandle.bind(this) })
	                ),
	                React.createElement(
	                    'section',
	                    { className: 'downView' },
	                    React.createElement(_AromaSegView.AromaSegView, { modeState: this.state.modeState, changeMode: this.changeModeHandle.bind(this) }),
	                    colorDom,
	                    React.createElement(
	                        'section',
	                        null,
	                        React.createElement(_AromaMistAndTimeView.AromaMistAndTimeView, { mist: this.state.mist, time: this.state.timeClose,
	                    ...
	                    React.createElement(
	                        Link,
	                        { to: '/PlanSettingMain' },
	                        React.createElement(_TimeShowView.TimeShowView, { showPlanSetting: this.state.showSetTime, planInfo: this.orderObj })
	                    )
	                ),
	                React.createElement(_ColorRingView.ColorRingView, { isShow: colorRingViewIsShow, disMissRingView: this.disMissRingView.bind(this), colorBar: this.state.color, selectColor: this.selectColorHandle.bind(this), submitColor: this.submitColorHandle.bind(this) }),
	                React.createElement(
	                    ...
	                    showOrderSet && React.createElement(
	                        'section',
	                        ...
	                        React.createElement(
	                            Link,
	                            { to: '/PlanSettingMain' },
	                            React.createElement(_CloseView.CloseView, { showPlanSetting: this.state.showSetTime, planInfo: this.orderObj })
	                        )
	                    )
	                )
	            );
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _CommonFun = __webpack_require__(92);

	var _DefinMarco = __webpack_require__(96);
	...
	var Store = exports.Store = Reflux.createStore({
	    ...
	    onGetRunData: function onGetRunData() {
	        ...
	        var url = _CommonFun.CommonFun.urlStr("GET", _DefinMarco.Macro.kHETHostName + _DefinMarco.Macro.kURLDeviceDomain + _DefinMarco.Macro.kURLDeviceRunDataGet, { deviceId: "CF2029A87083A6CC3B59CB4AD9F9ECA2", accessToken: "c452b642cb7744ac83f1ca5ec0735e63" });
	        het.get(url, {}, function (res) {
	            console.log(res);
	            _this.displayData = res;
	        });
	    },
	    onGetConfigData: function onGetConfigData() {
	        var url = _CommonFun.CommonFun.urlStr("GET", _DefinMarco.Macro.kHETHostName + _DefinMarco.Macro.kURLDeviceDomain + _DefinMarco.Macro.kURLDeviceConfigGet, { deviceId: "CF2029A87083A6CC3B59CB4AD9F9ECA2", accessToken: "c452b642cb7744ac83f1ca5ec0735e63" });
	        het.get(url, {}, function (res) {
	            console.log(res);
	        });
	    },
	    onGetDisplayData: function onGetDisplayData() {
	    ...
	    onChangeMode: function onChangeMode(mode) {
	        ...
	        if (mode === _AromaSegView.ModeState.ModeState1) {
	            sendObj = { "mode": _AromaSegView.ModeState.ModeState1, "updateFlag": 2 };
	        } else if (mode === _AromaSegView.ModeState.ModeState3) {
	            sendObj = { "mode": _AromaSegView.ModeState.ModeState3, "brightness": 50, "updateFlag": 6 };
	        }
	    ...
	    onChangeMist: function onChangeMist(value) {
	        var sendObj = { mist: value, updateFlag: 1 };
	        this.sendObjAndTrig(sendObj);
	    },
	    changeCloseTime: function changeCloseTime(value) {
	        var timeCloseH = parseInt(value / 60);
	        var timeCloseM = parseInt(value % 60);
	        var sendObj = { timeCloseH: timeCloseH, timeCloseM: timeCloseM, updateFlag: 8 };
	        this.sendObjAndTrig(sendObj);
	    },
	    onChangeSwitch: function onChangeSwitch(value) {
	        var sendObj = value === 0 ? { "brightness": 100, "mist": _AromaMistAndTimeView.MistState.MistStateHigh, "mode": _AromaSegView.ModeState.ModeState3, "updateFlag": 0x07 } : { "brightness": 0, "mist": _AromaMistAndTimeView.MistState.MistStateClose, "mode": _AromaSegView.ModeState.ModeState3, "updateFlag": 0x07 };
	        this.sendObjAndTrig(sendObj);
	    },
	    onGetRecentlyColor: function onGetRecentlyColor() {
	        var colorStr = "#" + this.displayData["red"].toTwoHex() + this.displayData["green"].toTwoHex() + this.displayData["blue"].toTwoHex();
	        this.trigger({ colorStr: colorStr });
	    },
	    onChangeOrderInfo: function onChangeOrderInfo(dic) {
	        var color = dic["orderColor"];
	        color = color.slice(1);
	        color = parseInt(color, 16);
	        var presetRun = $.parseIntInvalidToZero(dic["presetRunTime"]);
	        var sendObj = { "presetOpenH": $.parseIntInvalidToZero(dic["presetOpenH"]),
	    ...
	    sendObjAndTrig: function sendObjAndTrig(sendObj) {

	        het.send(sendObj, function () {
	            alert("succ");
	        }, function () {
	            alert("fail");
	        });
	            ...
	            return React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    React.createElement(
	                        ...
	                        React.createElement(
	                            "a",
	                            { href: "#", onTouchEnd: this.tapMode.bind(this), "data-val": Number(0) },
	                            "\u8F6E\u64AD\u6A21\u5F0F"
	                        )
	                    ),
	                    React.createElement(
	                        ...
	                        React.createElement(
	                            "a",
	                            { href: "#", onTouchEnd: this.tapMode.bind(this), "data-val": Number(1) },
	                            "\u5355\u8272\u6A21\u5F0F"
	                        )
	                    )
	                )
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Seting = function (_React$Component) {
	    (0, _inherits3.default)(Seting, _React$Component);

	    function Seting(props) {
	        (0, _classCallCheck3.default)(this, Seting);
	        return (0, _possibleConstructorReturn3.default)(this, (Seting.__proto__ || (0, _getPrototypeOf2.default)(Seting)).call(this, props));
	    }

	    (0, _createClass3.default)(Seting, [{
	        key: "render",
	        value: function render() {
	            return React.createElement(
	                "div",
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ColorSelectView = exports.ColorSelectView = function (_React$Component) {
	    ...
	    function ColorSelectView() {
	        ...
	        _this.colors = ['#ff7b7c', '#ffffff', '#fcaa6b', '#fcda6f', '#a0e674', '#59bdef', 'tapRingEvent'];
	        _this.index = 0;
	                ...
	                React.createElement(
	                    ...
	                    this.colors.map(function (value, index) {
	                        if (index < 6) {
	                            return React.createElement(
	                                'div',
	                                { style: { backgroundColor: value }, key: index, onTouchEnd: _this2.changeColorHandle.bind(_this2), 'data-val': index },
	                        ...
	                        return React.createElement(
	                            'div',
	                            { key: index, onTouchEnd: _this2.changeColorHandle.bind(_this2), 'data-val': index },
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _RingImgBase = __webpack_require__(105);
	...
	var ColorRingView = exports.ColorRingView = function (_React$Component) {
	    ...
	    (0, _createClass3.default)(ColorRingView, [{
	        ...
	        value: function componentDidMount() {
	            ...
	            var imgData = __webpack_require__(106);
	        ...
	        value: function caculateColor(e) {
	            ...
	            var colorStr = "#" + rgba[0].toString(16) + rgba[1].toString(16) + rgba[2].toString(16);
	            return colorStr;
	        }
	    }, {
	        key: 'getOffset',
	        ...
	        value: function render() {
	            ...
	            return React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    React.createElement('a', { href: '#', className: 'colorBar', style: { backgroundColor: this.props.colorBar } }),
	                    React.createElement(
	                        'div',
	                    ...
	                    React.createElement('a', { href: '#', className: 'closeBtn', onTouchEnd: this.disMiss.bind(this) })
	                )
	            );
	        }
	    }]);
	...
	var YYScale = exports.YYScale = function (_React$Component) {
	    ...
	    (0, _createClass3.default)(YYScale, [{
	        ...
	        value: function pos(value) {
	            var scale = ReactDOM.findDOMNode(this.refs["scale"]);
	            var cursor = ReactDOM.findDOMNode(this.refs["cursor"]);
	            var scaleValue = ReactDOM.findDOMNode(this.refs["scaleValue"]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var TimeShowView = exports.TimeShowView = function (_React$Component) {
	    ...
	    function TimeShowView(props) {
	        (0, _classCallCheck3.default)(this, TimeShowView);

	        var _this = (0, _possibleConstructorReturn3.default)(this, (TimeShowView.__proto__ || (0, _getPrototypeOf2.default)(TimeShowView)).call(this, props));

	        _this.showPlanSetting = props.showPlanSetting || false;
	        _this.planInfo = props.planInfo || { time: '09:30', color: "#ff0000", brightness: '50%', mist: '小雾' };
	        return _this;
	    }
	    ...
	    (0, _createClass3.default)(TimeShowView, [{
	        ...
	        value: function render() {
	            ...
	            if (this.showPlanSetting) {
	                timeDom = React.createElement(
	                    'div',
	                    { className: 'timeContainer1', onTouchEnd: this.tapHandle.bind(this) },
	                    React.createElement(
	                        'div',
	                        { className: 'row' },
	                        React.createElement(
	                    ...
	                    React.createElement(
	                        ...
	                        React.createElement('div', { style: { width: '24px', height: '24px', backgroundColor: '' + this.planInfo.color, display: 'inline-block' } })
	                    ),
	                    React.createElement(
	                        'div',
	                        { className: 'row' },
	                ...
	                timeDom = React.createElement(
	                    'span',
	                    { style: { height: '44px', color: '#64627f', fontSize: '16px', lineHeight: '44px', marginRight: '16px', float: 'right', marginLeft: 'auto' }, onTouchEnd: this.tapHandle.bind(this) },
	                    '\u672A\u8BBE\u7F6E'
	                );
	            }
	            return React.createElement(
...
/***/ function(module, exports, __webpack_require__) {
	...
	var CloseView = exports.CloseView = function (_React$Component) {
	    ...
	    function CloseView(props) {
	        ...
	        var _this = (0, _possibleConstructorReturn3.default)(this, (CloseView.__proto__ || (0, _getPrototypeOf2.default)(CloseView)).call(this, props));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _DatePickView = __webpack_require__(112);

	var _AromaMistAndTimeView = __webpack_require__(99);

	var _CommonView = __webpack_require__(113);

	var _SetColorRing = __webpack_require__(114);
	...
	var PlanSettingMain = exports.PlanSettingMain = function (_BaseComponent) {
	    ...
	    function PlanSettingMain(props) {
	        ...
	        _this.state = { orderColor: "#ff0000",
	            orderBrightness: "50%",
	            oderMist: "大雾",
	            presetRunTime: "不限时",
	            showRing: false, showBright: false, showMist: false, showRun: false };
	        _this.planInfo = { presetOpenH: 9,
	            presetOpenM: 30, orderColor: "#ff0000", orderBrightness: 50, orderMist: _AromaMistAndTimeView.MistState.MistStateHigh, presetRunTime: 0 };
	    ...
	    (0, _createClass3.default)(PlanSettingMain, [{
	        key: 'componentDidMount',
	        value: function componentDidMount() {
	            this.removeListen = _Store.Store.listen(this.listenHandle.bind(this));
	            het.setTitle('定时开启');
	            //获取最近使用的颜色
	            _Actions.Actions.getRecentlyColor();
	        }
	    }, {
	        key: 'componentWillUnmount',
	        value: function componentWillUnmount() {
	        ...
	        key: 'listenHandle',
	        value: function listenHandle(dic) {
	        ...
	        value: function submitRunTime(index) {
	            ...
	            var value = $.parseIntInvalidToZero(this.runTimeArr[index]);
	        ...
	        value: function setOrderComplete(e) {
	            e.preventDefault();
	            e.stopPropagation();
	            _Actions.Actions.changeOrderInfo(this.planInfo);
	            window.history.back();
	        }
	    }, {
	        key: 'render',
	        value: function render() {
	            ...
	            return React.createElement(
	                ...
	                React.createElement(_DatePickView.DatePickView1, { dateChange: this.dateChangeHandle.bind(this),
	                ...
	                React.createElement(
	                    ...
	                    React.createElement(
	                        ...
	                        React.createElement(
	                            ...
	                            React.createElement('div', { className: 'orderColor', style: { backgroundColor: '' + this.state.orderColor } }),
	                ...
	                React.createElement(
	                    ReactCSSTransitionGroup,
	                    {
	                        ...
	                        transitionEnterTimeout: 300,
	                        transitionLeaveTimeout: 300 },
	                    this.state.showBright && React.createElement(SetBrightness, { value: this.state.orderBrightness,
	                ...
	                React.createElement(
	                    ReactCSSTransitionGroup,
	                    {
	                        ...
	                        transitionEnterTimeout: 300,
	                        transitionLeaveTimeout: 300 },
	                    this.state.showMist && React.createElement(SetMistAndRunTime, { data: ["关闭", "大雾", "小雾", "睡眠"], cancleHandle: this.dismiss.bind(this),
	                ...
	                React.createElement(
	                    ReactCSSTransitionGroup,
	                    {
	                        ...
	                        transitionEnterTimeout: 300,
	                        transitionLeaveTimeout: 300 },
	                    this.state.showRun && React.createElement(SetMistAndRunTime, { data: ["不限时", "5", "10", "30", "60", "120"], cancleHandle: this.dismiss.bind(this),
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SetColorSelect = function (_React$Component2) {
	    ...
	    function SetColorSelect() {
	        ...
	        _this3.colors = ['#ff7b7c', '#ffffff', '#fcaa6b', '#fcda6f', '#a0e674', '#59bdef'];
	        _this3.index = 0;
	        return _this3;
	    }
