...
export const TimeSelect = React.createClass({
	...
	timearrInit:function(next){
		...
		if(next.defaultminute){
			let mindex = next.minutearr.indexOf(next.defaultminute);
			if(mindex!=-1){
				this.setState({
	...
	componentWillReceiveProps: function(next) {
		...
		if(next.hourstep!=this.props.hourstep || next.minhour!=this.props.minhour
			|| next.maxhour!=this.props.maxhour || next.arrayInit===true || this.props.defaultminute != next.defaultminute){
	...
	endrange:function(e){
		...
		let minutearr = this.props.minutearr;//分钟可选值数组
	...
	render: function() {
		...
		let timeLeftTitle = this.props.timeLeftTitle || ' ';
		...
		let minutearr = this.props.minutearr;
	    ...
	    return (
	    	<section style={{display:this.state.timeDisplay?"block":"none",opacity:this.state.showOpacity}} ref='timeSelect' className='timeSelect'>
		        ...
		        <section className="timeselect" onTouchMove={this.endDefault}>
		        	...
		        	<section className='time'>
		        		...
		        		<section className='timetext'>
		        			<span className='timeLeft'>{timeLeftTitle}</span>
		        			<span className='hour' style={{left:minuteshow?49+'%':53+'%',display:hourshow?'inline-block':'none'}}></span>
		        			<span className='minute' style={{display:minuteshow?'inline-block':'none',left:hourshow?66+'%':53+'%'}}></span>
		        			<span className={status}>{statusname}</span>
		        		</section>
		        		<section className='hourvalue flex-column' style={{top:hourtop+'%',left:minuteshow?41+'%':45+'%',display:hourshow?'':'none'}}>
