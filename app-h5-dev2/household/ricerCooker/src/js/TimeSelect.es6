...
export const TimeSelect = React.createClass({
	...
	timearrInit:function(next){
		//设置时间可选值数组
		let maxhour = parseInt(next.maxhour) || parseInt(this.props.maxhour) || 24;
		let minhour = parseInt(next.minhour) || parseInt(this.props.minhour) || 1;
	...
	submitclock:function(e){
		...
		if(typeof this.props.submitClock === 'function'){
			let hours=this.state.hourtime==1 ? '01' :this.state.hourtime;
		        ...
		        <section className="timeselect" onTouchMove={this.endDefault}>
		        	...
		        	<section className='time'>
		        		...
		        		<section className='timetext'>
		        			<span className='reminder'>预约</span>
