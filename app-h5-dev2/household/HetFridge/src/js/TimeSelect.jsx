...
export const TimeSelect = React.createClass({
	...
	timearrInit:function(next){
		...
		let maxminute = parseInt(next.maxmin)  || 60;
		...
		for(let i=0;i<7;i++){
			let date = new Date();
			date.setDate(date.getDate()-i);//获取AddDayCount天后的日期
			var y = date.getFullYear();
			var m = date.getMonth()+1;//获取当前月份的日期
			var d = date.getDate();
			let dataContent = y+"年"+m+"月"+d+"日";
			...
			<section ref="selecter" style={{display:this.state.timeDisplay?'block':'none',opacity:this.state.showOpacity}} ref='timeSelect' className='timeSelect'>
				...
				<section className="timeselect" onTouchMove={this.endDefault}>
					...
					<section className='time'>
						...
						<section  className='minutevalue flex-column' style={{top:minutetop+'%',display:minuteshow?'block':'none',left:hourshow?58+'%':33+'%'}}>
