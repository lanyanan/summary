...
/***/ function(module, exports, __webpack_require__) {
	...
	var Store = exports.Store = Reflux.createStore({
	    ...
	    onRepaint: function onRepaint(datas, type) {
	        ...
	        var isWisdomEye = (appData.runstatus === 1 || appData.runstatus === 4 || appData.runstatus === 5 || appData.runstatus === 6) && appData.wisdomeye === 1;
	     ...
	     * @param where                 点击的位置
	     */
	    onChangeMode: function onChangeMode(where, wisedonEyeStatus) {
	        //点击的位置
	        var _this = this;
	        ...
	        if (where === 0) {
	            ...
	            //打开智慧眼
	            if (appData.runstatus === 2) {
	                //如果当前是换气模式下, 亦既是 当前换气模式已打开, 点击智慧眼(打开智慧眼, 关闭换气)
	            ...
	            //打开换气开关
	            if ((appData.runstatus === 1 || appData.runstatus === 4 || appData.runstatus === 5 || appData.runstatus === 6) && appData.wisdomeye === 1) {
	                //如果当前是智慧眼模式下
	    ...
	    onStartVentilation: function onStartVentilation() {
	        var _this = this;
	        appData.wisdomeye = 2; //关闭智慧眼
	        appData.ventilation = 1; //打开换气开关
	        appData.updateFlag = het.hexUpFlag(2, 2, 2);
	        het.send(appData, function (data) {}, function (appData) {
	            het.toast("命令发送失败");
	        });
	        setDataTimer('runstatus', 'wisdomeye');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _DialogStyle = __webpack_require__(93);

	var _DialogButtonOne = __webpack_require__(94);
	...
	var App = function (_BaseComponent) {
	    ...
	    function App(props) {
	        ...
	        _this.submitDialogButtonOne = function () {
	        ...
	        _this.liveError = _this.liveError.bind(_this); //故障&错误
	        _this.clickIKnow = _this.clickIKnow.bind(_this);
	        _this.submitDialog = _this.submitDialog.bind(_this);
	        _this.cancelDialog = _this.cancelDialog.bind(_this);
	        _this.getDayHourDesc = _this.getDayHourDesc.bind(_this);
	        return _this;
	    }
	    ...
	    (0, _createClass3.default)(App, [{
	        ...
	        value: function changeMode(e) {
	            ...
	            _Actions.Actions.changeMode(where, this.state.wisdomeye);
	        }
	    }, {
	        key: 'getDayHourDesc',
	        value: function getDayHourDesc(time) {
	        ...
	        key: 'clickIKnow',
	        value: function clickIKnow(e) {
	            var status = this.state.isShowDialog;
	            this.setState({
	                isShowDialog: !status
	            });
	        }
	        ...
	        key: 'submitDialog',
	        value: function submitDialog() {
	            // console.log("弹窗 点击确定");
	            _Actions.Actions.startVentilation(); //打开换气
	        }
	    }, {
	        key: 'cancelDialog',
	        value: function cancelDialog() {
	            // console.log("弹窗 点击取消");
	            _Actions.Actions.hideDialog();
	        }
	    }, {
	        key: 'render',
	        value: function render() {
	            ...
	            var odor = this.state.odor;
	            var wisdomeye = this.state.wisdomeye;
	            var notcleantime = this.state.notcleantime;
	            ...
	            var onLineHintStatus = online == 2 ? "device-status hide" : "device-status";

	            //是否打开了照明
	            var isOpenLight = online == 1 && networkavailable == 1 && lightstatus === 1 && runstatus !== 7;
	            //是否打开了设备
	            var isOpenPower = online == 1 && networkavailable == 1 && runstatus !== 7;
	            //是否打开了智慧眼  true:打开;false:关闭
	            var isOpenWisdomEye = online == 1 && networkavailable == 1 && runstatus === 1 && wisdomeye === 1 && odor != 0; //设备在线 且有网 且待机状态下 且智慧眼打开 且气味值不为0
	            //是否打开了换气
	            var isOpenVentilation = online == 1 && networkavailable == 1 && runstatus === 2;
	            ...
	            var airStatusDiv = online == 2 || networkavailable == 2 || runstatus === 7 || odor === 0 ? "air-status hide" : "air-status";
	            var airStatus = odor !== undefined ? odor < 10 ? 0 : odor < 20 ? 1 : odor < 50 ? 2 : odor < 70 ? 3 : 4 : 5; //空气质量档位
	            var airStatusImg = ["../static/img/air-status-1.png", "../static/img/air-status-2.png", "../static/img/air-status-3.png", "../static/img/air-status-4.png", "../static/img/air-status-5.png", "../static/img/air-status-3.png"][airStatus];
	            var airStatusDesc = ["空气优", "轻度污染", "中度污染", "高度污染", "预警", "未知"][airStatus];
	            var workStatus = runstatus === 4 || runstatus === 5 || runstatus === 6 ? runstatus - 2 : 1;
	            var workStatusImg = "../static/img/device-screen-" + workStatus + ".jpg";
	            var lightingImg = isOpenLight ? "../static/img/mode-1-on.png" : "../static/img/mode-1-off.png";
	            var switchImg = online == 2 || runstatus === 7 ? "../static/img/mode-2-off.png" : "../static/img/mode-2-on.png";
	            var smartImg = isOpenWisdomEye ? "../static/img/mode-3-on.png" : "../static/img/mode-3-off.png";
	            var breathImg = isOpenVentilation ? "../static/img/mode-4-on.png" : "../static/img/mode-4-off.png";
	            var textColorLighting = isOpenLight ? "color-yellow" : "color-black";
	            var textColorSwitch = online == 2 || runstatus === 7 ? "color-black" : "color-yellow";
	            var textColorSmart = isOpenWisdomEye ? "color-yellow" : "color-black";
	            var textColorBreath = isOpenVentilation ? "color-yellow" : "color-black";
	            var textColseDesc = runstatus === 7 ? "开机" : "关机";
	            var lineStatusDesc = "";
	            ...
	            var title = "智慧眼提醒";
	            var message = "厨房空气质量差, 请开启烟机联动换气!";
	            // console.log("this.state.isShowAlert : "+this.state.isShowAlert);
	            if (online == 2 && appData.latestOnline != undefined && appData.latestOnline == 1) {
	                //上一次状态是在线 现在的状态是离线 给出弹窗提示
	            ...
	            if (runstatus !== 7 && appData.latestRunStatus != undefined && appData.latestRunStatus === 7 && notcleantime > 45 * 24) {
	                //上一次状态是烟机清洗中 现在的状态是不是清洗 给出弹窗提示
	                this.state.titleButtonOne = "清洗提醒";
	                this.state.contentButtonOne = '您已经' + this.getDayHourDesc(notcleantime) + '未清洗抽油烟机，请清洗烟机';
	                this.state.buttonContentOne = "我知道了";
	                this.state.isShowDialogButtonOne = true;
	            }
	            if (runstatus !== 3 && appData.latestRunStatus != undefined && appData.latestRunStatus === 3) {
	                //上一次状态是烟机清洗中 现在的状态是不是清洗 给出弹窗提示
	            ...
	            return React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    React.createElement(
	                        ...
	                        React.createElement('img', { src: workStatusImg, alt: '' }),
	                        React.createElement(
	                            'h5',
	                            { className: onLineHintStatus },
	                            lineStatusDesc
	                        ),
	                        React.createElement(
	                            'div',
	                            { className: airStatusDiv },
	                            React.createElement('img', { src: airStatusImg, alt: '' }),
	                            React.createElement(
	                                'span',
	                                null,
	                                airStatusDesc
	                    ...
	                    React.createElement(
	                        ...
	                        React.createElement(
	                            'div',
	                            { 'data-seat': '0', className: 'app-btns-child', onClick: changeMode,
	                                style: runstatus === 3 ? { opacity: .5 } : { opacity: 1 } },
	                            React.createElement('img', { src: lightingImg, alt: '' }),
	                            React.createElement(
	                                'h5',
	                                { className: textColorLighting },
	                                '\u7167\u660E'
	                            )
	                        ),
	                        React.createElement(
	                            'div',
	                            { 'data-seat': '1', className: 'app-btns-child', onClick: changeMode },
	                            React.createElement('img', { src: switchImg, alt: '' }),
	                            React.createElement(
	                                'h5',
	                                { className: textColorSwitch },
	                                textColseDesc
	                            )
	                        ),
	                        React.createElement(
	                            'div',
	                            { 'data-seat': '2', className: 'app-btns-child', onClick: changeMode,
	                                style: runstatus === 3 ? { opacity: .5 } : { opacity: 1 } },
	                            React.createElement('img', { src: smartImg, alt: '',
	                                style: odor == 0 ? { opacity: .3 } : runstatus === 3 ? { opacity: .5 } : { opacity: 1 } }),
	                            React.createElement(
	                                'h5',
	                                { className: textColorSmart,
	                                    style: odor == 0 ? { opacity: .3 } : runstatus === 3 ? { opacity: .5 } : { opacity: 1 } },
	                                '\u667A\u6167\u773C'
	                            )
	                        ),
	                        React.createElement(
	                            'div',
	                            { 'data-seat': '3', className: 'app-btns-child', onClick: changeMode,
	                                style: runstatus === 3 ? { opacity: .5 } : { opacity: 1 } },
	                            React.createElement('img', { src: breathImg, alt: '' }),
	                            React.createElement(
	                                'h5',
	                                { className: textColorBreath },
	                                '\u6362\u6C14'
	                            )
	                        ),
	                        React.createElement(
	                    ...
	                    React.createElement(_DialogStyle.DialogStyle, { show: this.state.isShowAlert, cancelClock: this.canceldia,
