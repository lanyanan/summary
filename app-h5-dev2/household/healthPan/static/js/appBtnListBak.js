...
/***/ function(module, exports) {
	...
	'local', 'slide', 'cancel', 'cancelSelect', 'submitSelect', 'selectPicker', 'closeSelector', 'toggleMode', 'selectMode', 'changeMode', 'preventJump']);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Store = exports.Store = Reflux.createStore({
	    ...
	    onRepaint: function onRepaint(datas) {
	        ...
	        //调试打印
	        //if(data.GanShaoBaoHu!=undefined) het.toast('干烧'+data.GanShaoBaoHu);
	        //if(data.ChuanGanQiKaiLuGuZhang!=undefined) het.toast('开路'+data.ChuanGanQiKaiLuGuZhang);
	        //if(data.ChuanGanQiDuanLuGuZhang!=undefined) het.toast('短路'+data.ChuanGanQiDuanLuGuZhang);
	        // if(data.GongZuoMoShiSheZhi!=undefined) het.toast('控制>>>>>>~>>>>模式'+data.GongZuoMoShiSheZhi);
	    },
	    ...
	    onSubmitSelect: function onSubmitSelect(h, m, where, live) {
	        if (live == true) {
	            ...
	            if (where == 1) {
	                ...
	                h == '武' && (liveSend.fire = 0, liveSend.HuoLiSheZhi = 0);
	                h == '文' && (liveSend.fire = 1, liveSend.HuoLiSheZhi = 1);
	    ...
	    onCloseSelector: function onCloseSelector(data) {
	        this.trigger({ selectshow: false });
	    },
	    ...
	    onChangeMode: function onChangeMode(data) {
	        ...
	        if (data.GongZuoMoShiSheZhi == 0) console.log('FLAG1', data.GongZuoMoShiSheZhi), updateFlag = het.hexUpFlag(3, 1, 2);
	        if (data.GongZuoMoShiSheZhi == 1) console.log('FLAG2', data.GongZuoMoShiSheZhi), updateFlag = het.hexUpFlag(0, 1, 2, het.hexUpFlag(3, 1, 2));
	        if (data.GongZuoMoShiSheZhi == 8) console.log('FLAG2', data.GongZuoMoShiSheZhi), updateFlag = het.hexUpFlag(3, 1, 2, het.hexUpFlag(4, 1, 2, het.hexUpFlag(5, 1, 2)));
	        data.updateFlag = updateFlag;
	        console.log('data------------', data);
	        //剔除火力
	        //data.GongZuoMoShiSheZhi != 7 && delete data.HuoLiSheZhi;
	        //除了协议中定义的字段可以发送，其他额外字段会被后台丢弃
	        het.send(data, function (data) {}, function (data) {
	            het.toast("命令发送失败");
	        });
	        //协议定义为拼音不便阅读，映射一遍，存到全局变量里trigger，物理返回键和回退操作都调用了local方法重新请求了一次缓存数据
	        ...
	        console.log('data------------', data);
	        //放弃治疗，用运行数据渲染，日了狗了，又要发控制数据又要发运行数据
	        //控制数据
	        appData.reservehour = data.YuYueShiJianSheZhiXiaoShi;
	        appData.reservemin = data.YuYueShiJianSheZhiFenZhong;
	        appData.fire = data.HuoLiSheZhi;
	    ...
	    onPreventJump: function onPreventJump(data) {
	        var me = this;
	        me.trigger(data);
	    }
	});
	    ...
	    _createClass(App, [{
	        ...
	        value: function changeMode(e) {
	            ...
	            appData.sendArray = {
	                ...
	                YunXingGongZuoMoShi: appData.selectMode
	            };
	        ...
	        value: function liveError() {
	            ...
	            if (this.state.dryburning == 1) {
	                return '主人，请在锅内添加适量的水。养生锅会自动返回待机状态！';
	            }
	        ...
	        value: function render() {
	            ...
	            var modeName = this.state.modeName != undefined ? this.state.modeName : this.state.modeName;
	            //运行数据相对较慢并未用于渲染
	            var runningMode = this.state.runningMode != undefined ? this.state.runningMode : '';
	            var runningModeName = this.state.runningModeName != undefined ? this.state.runningModeName : 0;
	            //离线&故障
	            ...
	            //故障时样式
	            var runningOk = online != 2 && networkavailable != 2 && hightemperature != 1 && dryburning != 1 && opencircuit != 1 && shortcircuit != 1 ? true : false;
	            ...
	            //console.log(online,networkavailable,hightemperature,dryburning,opencircuit,shortcircuit,runningOk.toString())
	            return React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    React.createElement(
	                        'article',
	                        { 'data-mode': '7', className: mode == 7 && runningOk ? 'flex-cell triggered' : 'flex-cell', onClick: mode == 7 && runningOk ? '' : this.changeMode, style: runningOk ? { opacity: 1 } : { opacity: .5 } },
	                    ...
	                    React.createElement(
	                        'article',
	                        { 'data-mode': '5', className: mode == 5 && runningOk ? 'flex-cell triggered' : 'flex-cell', onClick: mode == 5 && runningOk ? '' : this.changeMode, style: runningOk ? { opacity: 1 } : { opacity: .5 } },
	                    ...
	                    React.createElement(
	                        'article',
	                        { 'data-mode': '6', className: mode == 6 && runningOk ? 'flex-cell triggered' : 'flex-cell', onClick: mode == 6 && runningOk ? '' : this.changeMode, style: runningOk ? { opacity: 1 } : { opacity: .5 } },
	                    ...
	                    React.createElement(
	                        'article',
	                        { 'data-mode': '4', className: mode == 4 && runningOk ? 'flex-cell triggered' : 'flex-cell', onClick: mode == 4 && runningOk ? '' : this.changeMode, style: runningOk ? { opacity: 1 } : { opacity: .5 } },
