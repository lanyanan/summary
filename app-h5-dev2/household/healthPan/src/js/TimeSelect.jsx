				...
				<section className="timeselect" onTouchMove={this.endDefault}>
					...
					<section className='time'>
						...
						<section className='timetext'>
							<span style={{left:4+'%'}} className={statusshow?'status show':'status'}>{'养生锅将在'}</span>
							...
							<span className={statusshow?'status show':'status'}>{'后启动'||statusname}</span>
						</section>
						<section className='hourvalue flex-column' style={{top:hourtop+'%',left:minuteshow?25+'%':45+'%',display:hourshow?'block':'none'}}>
