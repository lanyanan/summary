	    	...
	    	<section style={{visibility:this.state.timeDisplay?"initial":"hidden",opacity:this.state.showOpacity}} ref='timeSelect' className='timeSelect'>
		        ...
		        <section className="timeselect" onTouchMove={this.endDefault}>
		        	<section className='selectbtn flex'>
		        		<span className='' onTouchEnd={this.cancelclock}>关闭定时</span>
		        		<span className='' onTouchEnd={this.submitclock}>开启定时</span>
		        	</section>
		        	...
		        	<section className='time'>
		        		...
		        		<section className='timetext'>
		        			<span className='hour' style={{left:minuteshow?33+'%':53+'%',display:hourshow?'inline-block':'none'}}>小时后关机
</span>
		        		...
		        		<section className='hourvalue flex-column' style={{top:hourtop+'%',left:minuteshow?25+'%':45+'%',display:hourshow?'':'none'}}>
		        			<span className='line1'>{(hourindex-3)<0?hourarr[hourarr.length-3]:hourarr[hourindex-3]}</span>
		        			<span className='line1'>{(hourindex-2)<0?hourarr[hourarr.length-2]:hourarr[hourindex-2]}</span>
		        			<span className='line2'>{(hourindex-1)<0?hourarr[hourarr.length-1]:hourarr[hourindex-1]}</span>
		        			<span className='line3'>{hourarr[hourindex]}</span>
		        			<span className='line2'>{(hourindex+1)>=hourarr.length?hourarr[hourindex-hourarr.length+1]:hourarr[hourindex+1]}</span>
		        			<span className='line1'>{(hourindex+2)>=hourarr.length?hourarr[hourindex-hourarr.length+2]:hourarr[hourindex+2]}</span>
		        			<span className='line1'>{(hourindex+3)>=hourarr.length?hourarr[hourindex-hourarr.length+3]:hourarr[hourindex+3]}</span>
		        		</section>
