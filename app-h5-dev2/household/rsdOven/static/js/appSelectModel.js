...
/***/ function(module, exports, __webpack_require__) {
	...
	var Store = exports.Store = Reflux.createStore({
	    ...
	    onRepaint: function onRepaint(datas) {
	        ...
	        if (!!data.cBookingTimeSetHour) AppData.cBookingTimeSetHour = data.cBookingTimeSetHour;
	        if (!!data.cBookTimeSetMin) AppData.cBookTimeSetMin = data.cBookTimeSetMin;
	        if (!!data.cStoveMode) AppData.cStoveMode = data.cStoveMode;
	        if (!!data.cWorkTimeHour) AppData.cWorkTimeHour = data.cWorkTimeHour;
	        if (!!data.cWorkTimeMin) AppData.cWorkTimeMin = data.cWorkTimeMin;
	        if (!!data.cHotWindSw) AppData.cHotWindSw = data.cHotWindSw;
	        if (!!data.cCancle) AppData.cCancle = data.cCancle;
	        if (!!data.cWorkSet) AppData.cWorkSet = data.cWorkSet;
	        if (!!data.cConfirm) AppData.cConfirm = data.cConfirm;
	        if (!!data.updateFlag) AppData.updateFlag = data.updateFlag;
	        //运行字段
	        if (!!data.onoff) {
	            AppData.cOnoff = data.onoff;data.cOnoff = data.onoff;AppData.onoff = data.onoff;data.loading = 2;AppData.loading = 2;
	        };
	        if (!!data.workStatus) AppData.workStatus = data.workStatus;
	        if (!!data.HotWindStatus) {
	            AppData.HotWindStatus = data.HotWindStatus;
	            AppData.cHotWindSw = data.HotWindStatus;
	            data.cHotWindSw = data.HotWindStatus;
	        };
	        if (!!data.SetTemp) AppData.SetTemp = data.SetTemp;
	        if (!!data.curTemp) AppData.curTemp = data.curTemp;
	        if (!!data.SetTempHigh) AppData.SetTempHigh = data.SetTempHigh;
	        if (!!data.setTempLow) AppData.setTempLow = data.setTempLow;
	        if (!!data.curTempHigh) AppData.curTempHigh = data.curTempHigh;
	        if (!!data.curTempLow) AppData.curTempLow = data.curTempLow;
	        if (!!data.setTimeHour) AppData.setTimeHour = data.setTimeHour;
	        if (!!data.setTimeMin) AppData.setTimeMin = data.setTimeMin;
	        if (!!data.leftTimeHour) AppData.leftTimeHour = data.leftTimeHour;
	        if (!!data.leftTimeMin) AppData.leftTimeMin = data.leftTimeMin;
	        if (!!data.setBookingTimeHour) AppData.setBookingTimeHour = data.setBookingTimeHour;
	        if (!!data.setBookingtimeMin) AppData.setBookingtimeMin = data.setBookingtimeMin;
	        if (!!data.leftbookingTimeHour) AppData.leftbookingTimeHour = data.leftbookingTimeHour;
	        if (!!data.leftBookingtimeMin) AppData.leftBookingtimeMin = data.leftBookingtimeMin;
	        //错误字节字段
	        if (!!data.senseError) AppData.senseError = data.senseError; //传感器错误
	    ...
	    onCancelElm: function onCancelElm() {
	        ...
	        if (parseInt(AppData.cFunSel) != 0) {
	            AppData.cCancle = "01";
	            AppData.cStoveMode = "00";
	            AppData.updateFlag = het.hexUpFlag(2, 1, 2, het.hexUpFlag(10, 1, 2));
	        } else {
	            AppData.cStoveMode = "00";
	            AppData.updateFlag = het.hexUpFlag(2, 1, 2);
	        }
	        het.send(AppData, function (data) {
	            //取消之后手动恢复所有运行工作状态
	    ...
	    onModeStart: function onModeStart(setTimeHour, setTimeMin, SetTemp, WorkModeSelect, setBookingTimeHour, setBookingtimeMin) {
	        ...
	        var cWorkhour = date.getHours() + setTimeHour;
	        var cWorkminute = date.getMinutes() + setTimeMin;

	        if (setTimeHour != 0 || setTimeMin != 0) {
	            //AppData.cWorkTimeHour = decToHex((cWorkminute>59?cWorkhour+1:cWorkhour)%24);
	            AppData.cWorkTimeHour = setTimeHour;
	            AppData.cWorkTimeMin = setTimeMin;
	            //AppData.cWorkTimeMin = decToHex(cWorkminute>59?(cWorkminute-60):cWorkminute);
	            AppData.updateFlag = het.hexUpFlag(7, 1, 2, het.hexUpFlag(8, 1, 2, AppData.updateFlag));
	        }
	        if (SetTemp != 0) {
	            //AppData.cWorkSet = "00" + decToHex(SetTemp);
	            AppData.cWorkSet = SetTemp;
	            AppData.updateFlag = het.hexUpFlag(5, 2, 2, AppData.updateFlag);
	        }
	        //let hour = date.getHours()+setBookingTimeHour;
	        //let minute = date.getMinutes()+setBookingtimeMin;
	        if (setBookingTimeHour != 0 || setBookingtimeMin != 0) {
	            ...
	            AppData.updateFlag = het.hexUpFlag(3, 1, 2, het.hexUpFlag(4, 1, 2, AppData.updateFlag));
	        } else {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _RanTime = __webpack_require__(13);
