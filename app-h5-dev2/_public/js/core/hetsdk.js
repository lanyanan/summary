...
window.het = (function(){
    ...
    function HET(){
            ...
            for (var k in settings.filter) {
                try {
                    switch (settings.filter[k]) {
                        case 0: // 只取控制数据
                            if (type !== 0) {delete data[k];}
                        break;
                        case 1: // 只取运行数据
                            if (type !== 1) {delete data[k];}
                        break;
                        default:
                            if (!settings.filter[k](type, data)) {delete data[k];}
                    }
                } catch (err) {
                    // console.log(err);
                }
        ...
        function bin2DecArray(binString){
            var arr = [], matt=/^(\d*)(\d{8})$|^\d{1,7}$/, str=binString.replace(/[^01]/g, '');
            while (str.length) {
        ...
        var web = {
            ...
            error : function(data, callbackId){
                // data = typeof data === "string" ? JSON.parse(data) : data;
                return execCallback(callbackId, Array.prototype.slice.call(arguments, 0, -1));
            },
            httpResponseSuccess : function(data, callbackId) {
                return execCallback(callbackId, Array.prototype.slice.call(arguments, 0, -1));
            },
            httpResponseError : function(data, callbackId) {
                return execCallback(callbackId, Array.prototype.slice.call(arguments, 0, -1));
            },
