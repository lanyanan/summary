	    ...
	    return (
	    	<section style={{display:this.state.timeDisplay?"block":"none"}} ref='timeSelect' className='timeSelect'>
		        ...
		        <section className="timeselect" onTouchMove={this.endDefault} style={{bottom:this.state.timeDisplay?"0":"-26rem"}}>
		        	...
		        	<section className="timedescription">
	        			<span className="hour-des">速度</span>
	        			<span className="minute-des">时间</span>
	        			<span className="second-des">旋转方向</span>
	        		</section>
		        	<section className='time'>
		        		<section data-type='hour' style={{width:minuteshow?'33%':'100%',display:hourshow?'inline-block':'none'}}
		        			onTouchStart={this.startrange} onTouchMove={this.moverange}
		        			onTouchEnd={this.endrange}  className='hour'>
		        		</section>
		        		<section  data-type='minute' style={{display:minuteshow?'inline-block':'none',width:hourshow?'33%':'100%',left:hourshow?'33%':'0%'}}
		        			onTouchStart={this.startrange} onTouchMove={this.moverange}
		        			onTouchEnd={this.endrange} className='minute'>
		        		</section>
		        		<section  data-type='second' style={{display:secondshow?'inline-block':'none',width:hourshow?'33%':'100%',left:hourshow?'66%':'0%'}}
		        			onTouchStart={this.startrange} onTouchMove={this.moverange}
		        			onTouchEnd={this.endrange} className='second'>
		        		</section>
		        		<section className='timetext'>
		        			<span className='hour' style={{left:minuteshow?0+'%':53+'%',display:hourshow?'inline-block':'none'}}></span>
		        			<span className='minute' style={{display:minuteshow?'inline-block':'none',left:hourshow?53+'%':53+'%'}}>秒</span>
		        			<span className='second' style={{display:secondshow?'inline-block':'none',left:hourshow?66+'%':53+'%'}}></span>
		        			<span className='status'>{statusname}</span>
		        		</section>
		        		<section className='hourvalue flex-column' style={{top:hourtop+'%',left:minuteshow?15+'%':45+'%',display:hourshow?'':'none'}}>
		        			<span className={(hourindex-3)<0?'line4':'line4'}>{(hourindex-3)<0?'':hourarr[hourindex-3]}</span>
		        			...
		        			<span className={(hourindex+3)>=hourarr.length?'line4':'line4'}>{(hourindex+3)>=hourarr.length?'':hourarr[hourindex+3]}</span>
		        		</section>
		        		<section  className='minutevalue flex-column' style={{top:minutetop+'%',display:minuteshow?'':'none',left:hourshow?45+'%':45+'%'}}>
			        		<span className={(minuteindex-3)<0?'line4':'line4'}>{(minuteindex-3)<0?'':minutearr[minuteindex-3]}</span>
		        			...
		        			<span className={(minuteindex+3)>=minutearr.length?'line4':'line4'}>{(minuteindex+3)>=minutearr.length?'':minutearr[minuteindex+3]}</span>
		        		</section>
		        		<section  className='secondvalue flex-column' style={{top:secondtop+'%',display:secondshow?'':'none',left:(hourshow && minuteshow)?64+'%':45+'%'}}>
			        		<span className={(secondindex-3)<0?'line4':'line4'}>{(secondindex-3)<0?'':secondarr[secondindex-3]}</span>
		        			<span className={(secondindex-2)<0?'line4':'line1'}>{(secondindex-2)<0?'':secondarr[secondindex-2]}</span>
		        			<span className={(secondindex-1)<0?'line4':'line2'}>{(secondindex-1)<0?'':secondarr[secondindex-1]}</span>
		        			<span className='line3'>{secondarr[secondindex]}</span>
		        			<span className={(secondindex+1)>=secondarr.length?'line4':'line2'}>{(secondindex+1)>=secondarr.length?'':secondarr[secondindex+1]}</span>
		        			<span className={(secondindex+2)>=secondarr.length?'line4':'line1'}>{(secondindex+2)>=secondarr.length?'':secondarr[secondindex+2]}</span>
		        			<span className={(secondindex+3)>=secondarr.length?'line4':'line4'}>{(secondindex+3)>=secondarr.length?'':secondarr[secondindex+3]}</span>
		        		</section>
		        	</section>
