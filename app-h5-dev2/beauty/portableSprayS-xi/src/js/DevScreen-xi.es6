...
export class DevScreen extends BaseComponent{
    ...
    render() {
        var tips = [
            (<span>为使补水喷雾仪的使用效果更好，推荐您先使用测肤仪进行肤质测试...<a href="cbeauty://cbeauty_skintest">去测试肌肤&gt;&gt;</a></span>),
            (<span>您当前为{this.baseData.skins[this.props.skinType5]}肤质，推荐您使用{this.baseData.modes[this.props.mode]}！美丽女人是养出来的~</span>),
            (<span>您使用了补水喷雾仪后，脸部皮肤水分提升了{this.props.waterTrend}%，请继续保持~</span>)
        ];

        return (
            <section className="screen">
                {/*<img className="logo-xi" src="../static/img/lightBeauty/logo@2x.png" />*/}
                <div className="pic">
