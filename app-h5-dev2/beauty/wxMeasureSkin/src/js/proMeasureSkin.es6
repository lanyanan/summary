...
// 创建React组件
export class ProMeasureSkin extends BaseComponent {
    constructor(props) {
        super(props);
        this.state = {
        ...
        Actions.clearTest(this.state.part);//初始化测试状态
        Actions.deviceInfo();
        setTimeout(function(){
            Actions.setPart(this.state.part,'pro');
        }.bind(this),1000);
    }
    componentDidMount() {
    ...
    useProTest(e) {//护肤后测试
        ...
        e.preventDefault();
        let afterPro = ReactDOM.findDOMNode(this.refs.afterPro);
        afterPro.click();
    }
    render() {
        ...
        return (
            <div className="padding50">
                ...
                <div className="footer">
                    ...
                    <a href="##" onTouchTap={this.useProTest.bind(this)} className={status == 5 ? '':'btn-active'}>护肤后测试</a>
                </div>
                <Link ref="afterPro" to={'/afterprotest/'+part+'/'+this.state.productId+'/'+this.state.lastInsertId+'/'+this.state.water+'/'+this.state.oil+'/'+this.state.elasticity + '?openid=' + getQueryString('openid')||getCookie('openid')} />
