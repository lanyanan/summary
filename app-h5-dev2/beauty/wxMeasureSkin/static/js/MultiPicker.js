	...
	MultiPicker.prototype = {
		...
		generateArrData: function (targetArr) {
			...
			loop(0, targetArr.length, function (i) {
				tempArr.push({
					"regionId": targetArr[i].regionId,
					"regionName": targetArr[i].regionName
				})
			});
			return tempArr;
		},
		checkArrDeep: function (parent) {
			var _this = this;
			if ('subRegion' in parent && parent.subRegion!==null && parent.subRegion.length > 0) {
				_this.jsonArr.push(_this.generateArrData(parent.subRegion));
				_this.checkArrDeep(parent.subRegion[0]);
			}
		...
		insertLiArr: function (targetUl, arr) {
			...
			loop(0, arr.length, function (i) {
				html += '<li data-id="' + arr[i].regionId + '">' + arr[i].regionName + '</li>';
			});
		...
		initReady: function (idx, target) {
			...
			loop(0, _this.idxArr.length, function (i) {
				...
				_this.resultArr.push({
					"regionId": _this.jsonArr[i][_this.distance[i] / _this.liHeight + 2].regionId,
					"regionName": _this.jsonArr[i][_this.distance[i] / _this.liHeight + 2].regionName,
				});
			});
		...
		checkRange: function (idx) {
			...
			loop(0, idx + 1, function (i) {
				...
				tempObj   = i == 0 ? tempObj[targetIdx] : tempObj.subRegion[targetIdx];
			});
