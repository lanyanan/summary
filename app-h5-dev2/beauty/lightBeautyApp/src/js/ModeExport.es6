 ...
 * @prop {function} cancelExt 点击取消触发回调函数(无默认,传入空值或者非法值时console提示)
 * @prop {function} submitExt 点击确认触发回调函数(无默认,传入空值或者非法值时console提示)
...
export const ModeExport = React.createClass({
	...
	cancelExt:function(e){
		//取消选择
		if(typeof this.props.cancelExt === 'function'){
			this.props.cancelExt();
		}else{
			console.log('error:the cancel callback is not a function');
		}
	},
	submitExt:function(e){
		//确认提交时间
		if(typeof this.props.submitExt === 'function'){
			this.props.submitExt(this.state.hourtime,this.state.minutetime);
		}else{
	...
	render: function() {
	    ...
	    return (
	    	<section style={{visibility:this.state.timeDisplay?"initial":"hidden",opacity:this.state.showOpacity}} ref='timeSelect' className='timeSelect'>
		    	<section onTouchEnd={this.cancelExt}></section>
		        <section className="timeselect" onTouchMove={this.endDefault}>
		        	...
		        	<section className='selectbtn flex'>
		        		<span className='flex-cell' onTouchEnd={this.cancelExt}>取消</span>
		        		<span className='flex-cell' onTouchEnd={this.submitExt}>确定</span>
		        	</section>
		        	<section className='time'>
		        		...
		        		<section className='timetext'>
		        			...
		        			<span className='minute' style={{display:minuteshow?'inline-block':'none',left:hourshow?66+'%':53+'%'}}>档</span>
