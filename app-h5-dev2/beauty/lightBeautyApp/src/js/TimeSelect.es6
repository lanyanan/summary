...
export const TimeSelect = React.createClass({
	...
	componentWillReceiveProps: function(next) {
		...
		if(next.hourstep!=this.props.hourstep || next.minhour!=this.props.minhour
			...
			|| next.defaulthour != this.props.defaulthour || next.defaultminute != this.props.defaultminute){
	...
	render: function() {
		...
		let minuteText = this.props.minuteText;
	    return (
	    	<section style={{display:this.state.timeDisplay?"block":"none"}} ref='timeSelect' className='timeSelect'>
		        ...
		        <section className="timeselect" onTouchMove={this.endDefault} onTouchStart={this.endDefault}>
		        	...
		        	<section className='time'>
		        		...
		        		<section className='timetext'>
		        			...
		        			<span className='minute' style={{display:minuteshow?'inline-block':'none',left:hourshow?66+'%':55+'%'}}>{minuteText}</span>
{/*		        			<span className='status'>{statusname}</span>
*/}		        		</section>
		        		<section className='hourvalue flex-column' style={{top:hourtop+'%',left:minuteshow?20+'%':45+'%',display:hourshow?'':'none'}}>
