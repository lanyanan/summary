 ...
 * @prop {function} canceler 点击取消触发回调函数(无默认,传入空值或者非法值时console提示)
 * @prop {function} submiter 点击确认触发回调函数(无默认,传入空值或者非法值时console提示)
...
export const Mode = React.createClass({
	...
	canceler:function(e){
		//取消选择
		if(typeof this.props.canceler === 'function'){
			this.props.canceler();
		}else{
	...
	submiter:function(e){
		//确认提交时间
		if(typeof this.props.submiter === 'function'){
			this.props.submiter(this.state.hourtime,this.state.minutetime);
		}else{
	...
	render: function() {
	    ...
	    return (
	    	<section style={{visibility:this.state.timeDisplay?"initial":"hidden",opacity:this.state.showOpacity}} ref='timeSelect' className='timeSelect'>
		    	<section onTouchEnd={this.canceler}></section>
		        <section className="timeselect" onTouchMove={this.endDefault}>
		        	...
		        	<section className='selectbtn flex'>
		        		<span className='flex-cell' onTouchEnd={this.canceler}>取消</span>
		        		<span className='flex-cell' onTouchEnd={this.submiter}>确认</span>
		        	</section>
