...
export const TimeSelect = React.createClass({
	...
	componentWillReceiveProps: function(next) {
		...
		if(next.hourstep!=this.props.hourstep || next.minhour!=this.props.minhour || next.ArrayInit===true
			|| next.maxhour!=this.props.maxhour  || next.maxminute!=this.props.maxminute||next.defaultminute!=this.props.defaultminute){
	...
	render: function() {
		...
		let selectTipShow = this.props.title?true:false;
		let selectTip = this.props.title || '';  
	    ...
	    return (
	    	<section style={{display:(this.state.timeDisplay)?"block":"none"}} ref='timeSelect' className='timeSelect'>
		    	<section ></section>
		        <section className="timeselect" onTouchMove={this.endDefault} style={{bottom:this.state.timeDisplay? 0 :"-20rem"}}>
		        	<section className='selectbtn flex'>
		        		<span onTouchEnd={this.cancelclock}>取消</span>
		        		<span onTouchEnd={this.submitclock}>确认</span>
		        	</section>
		        	<section className='time'>
		        		...
		        		<section className='timetext'>
		        			...
		        			<span className='minute' style={{display:minuteshow?'inline-block':'none',left:hourshow?66+'%':53+'%'}}>{this.props.unit}</span>
		        	...
		        	<section className='selectTip flex' style={selectTipShow?{}:{display:'none'}}>
		        		<span className='tipWord'>推荐温度为{selectTip}°C</span>
		        	</section>
		        </section>
	        </section>
	    );
