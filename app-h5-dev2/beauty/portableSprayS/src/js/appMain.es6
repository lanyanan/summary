...
class App extends BaseComponent{
    ...
    render() {
        ...
        var tips = [
            (<span>为使便携喷雾仪S的使用效果更好，推荐您先使用测肤仪进行肤质测试...<a href="cbeauty://cbeauty_skintest">去测试肌肤&gt;&gt;</a></span>),
            (<span>您当前为{this.baseData.skins[skinType]}肤质，推荐您使用{this.baseData.modes[recommendMode]}！美丽女人是养出来的~</span>),
            (<span>您使用了喷雾仪后，脸部皮肤水分提升了{waterTrend}%，请继续保持~</span>)
        ];

        return (
            <div className="app-body flex-column" onTouchMove={this.handlerTouchMove.bind(this)}>
                <div id="panel-scroller" className="flex-cell">
                    <section className="screen-ctn">
                        <DevScreen onlineStatus={this.state.onlineStatus}/>
                        <div className="tip">{tips[skinDataCode]}</div>
                        {+busiSwitch==1?(
                            <div className="gear-choose" style={slideStyle} data-busi="1" onTouchEnd={this.handlerTounch.bind(this)}>
                                <div className="gear-txt">自动</div>
                                <div className="gear-circle"></div>
                            </div>):(
                            <div className="gear-choose" style={slideStyle} data-busi="0" onTouchEnd={this.handlerTounch.bind(this)}>
                                <div className="gear-txts">手动</div>
                                <div className="gear-circle gear-circles"></div>
                            </div>
                        )}
	                    <div className="model-info clearfix">
							<div className="model" onTouchEnd={this.changeMode.bind(this)}>
								<p className="font-b" style={activeStyle}><a href="javascript:">{+busiSwitch==0 ? this.baseData.modes[currentRunMode] : this.baseData.modes[mode]}</a></p>
								<p className="font-b" style={nonActiveStyle}><span>{+busiSwitch==0 ? this.baseData.modes[currentRunMode] : this.baseData.modes[mode]}</span></p>
								<p className="font-s">模式选择</p>
							</div>
							<div className="info">
								<p className="font-b">
									<a className="ico-left" style={icoLeftDisplay} href="javascript:void(0);" onTouchEnd={this.timeUpAndDown.bind(this, 0)}></a>
									{+busiSwitch==0 ? currentRunTime : runTime}S
									<a className="ico-right" style={icoRightDisplay}  href="javascript:void(0);" onTouchEnd={this.timeUpAndDown.bind(this, 1)}></a>
								</p>
								<p className="font-s">补水时间</p>
							</div>
						</div>
                        {this.state.electricity <= 4 && this.state.chargeStatus < 2 && this.state.onlineStatus!=2 ? (<div className="battery">电量不足，请充电</div>) : ""}
                    </section>
                </div>
                <div id="footer">
