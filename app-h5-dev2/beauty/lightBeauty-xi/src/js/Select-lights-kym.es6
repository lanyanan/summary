...
export class lights extends BaseComponent{
    constructor(props) {
            super(props);
            this.state = {};
            this.items = [
            ...
            <div className="select-modes select-lights">
                ...
                <menu>
                    {this.items.map(function(item,index){
                        return <input key={index} className={item.id == value ? "active" : ""} type="button" value={item.text} data-value={item.id} onTouchEnd={this.handlerClick.bind(this)} />;
                    }.bind(this))}
                </menu>
            </div>
        );
