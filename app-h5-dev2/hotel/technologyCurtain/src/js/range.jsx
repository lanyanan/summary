    ...
    render : function() {
        ...
        return (
            <div className="__range">
                <label ref="wrap">
                    <input type="range"     min={this.min()} max={this.max()} onTouchEnd={this.selectRange}   onChange={this.handlerChange} value={value} disabled={this.props.disabled ? "disabled" : ""} />
                    <i ref="cursor" className="cursor"   id="glide"   ondragenter="return false"  >{value}</i>
                </label>
            </div>
        );
    }
