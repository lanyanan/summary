const CalendarHeader1 = React.createClass({
...
var CalendarBody1 = React.createClass({
    ...
    selectDate:function(e){
        var curLi = e.target,
            ...
            dateIpt=document.getElementsByClassName("dateIpt"),
            carlendarBody=document.getElementsByClassName("calendarBorder1")[0],
            _display=carlendarBody.style.display;
        var index = (curLi.parentNode.parentNode.parentNode.parentNode.getAttribute("data-key"));
            if(curDate==null){

            }else{
                dateIpt[index].value = curDate;
            }
            let allLi = curLi.parentNode.children;
    ...
    render:function(){
        ...
        var node2 = arry2.map(function(item,index){
            ...
            return (curday.indexOf(item)>=0)?<li key={index} data-date={year+"-"+month+"-"+_fItem} style={{"backgroundColor": "green"}} onTouchEnd={_this.selectDate}>{item}</li>: <li key={index} style={{"color": "#b1adad"}} data-date={year+"-"+month+"-"+_fItem}>{item}</li>
        })
        return(
            <div>
                ...
                <div className="CalendarDay1" ref="CalendarDay1"><ul >{node1}{node2}</ul></div>
            </div>
        )
    }
});
export const CalendarControl1 = React.createClass({
    ...
    handleFilterUpdate: function(filterYear,filterMonth) {
        ...
        this.props.getDays(filterYear,filterMonth)
    },
    ...
    render:function(){
        ...
        return(
            <div data-key={this.props.datakey} className="calendarBorder1" ref="carlendarBody1" >
                <CalendarHeader1 
