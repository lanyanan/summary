...
var CalendarHeader = React.createClass({
            ...
            handleLeftClick:function(){
                var devId = this.props.deviceId;
                // console.log(devId,'*****************************');
                //$('li[data-date]').removeClass('hasdata');
                ...
                var dateStr = year+'-'+newMonth+'-1';
                Actions.getDateTime(devId,dateStr);
            },
...
var CalendarBody = React.createClass({
    ...
    mixins: [Reflux.listenTo(Store,"onGetDateTime")],
    getInitialState:function(){
     return {
         dateList: []
     }
    ...
    getDataDate: function(date){
        var devId = this.props.deviceId;
        var _this = this;
        var url = 'http://weixin.clife.cn/clife-wechat-test/wechat/hotel/mattress/getMonthDateList?deviceId='+devId+'&date='+date;
        var dateList=[];
        $.ajax({
            url: url,
            ...
            success: function(r){
                if(r.code==0){
                    ...
                    _this.setState({dateList: dateList})
                }
            }
        });
    },
    ...
    render:function(){
        ...
        return(
                <div className="calendarBorder" ref="carlendarBody">
                    <CalendarHeader deviceId={devId} year={this.state.year} month={this.state.month} day={this.state.day} updateFilter={this.handleFilterUpdate}/>
                    <CalendarBody deviceId={devId} year={this.state.year} month={this.state.month} day={this.state.day} callbackParent={this.onChildChanged} callback={this.onChildChangeCal}/>
                </div>
