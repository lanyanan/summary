    ...
    return new Promise(function(resolve, reject){
        ...
        function stamp(cb){
            return gulp.src([tmpPath + 'static/**','!' + tmpPath + 'static/images/**']).
                ...
                on('end', function(){
                    return gulp.src([tmpPath + '../rev/*.json', tmpPath + 'static/**/*.css']).
                        pipe(revCollector({
                            replaceReved: true
                        })).
                        // 修改为 ?v=stamp 形式
...
function initProj(){
    return new Promise(function(resolve, reject){
        var s1 = 4;
        packJS().on('end', function(){if(--s1===0)resolve('done');});
        buildLess().on('end', function(){if(--s1===0)resolve('done');});
        buildSass().on('end', function(){if(--s1===0)resolve('done');});
        buildCSS().on('end', function(){if(--s1===0)resolve('done');});
    }).then(function(){
        console.log('初始化项目完成');
    });
}
...
function buildSass(src, dist){
    ...
    return {on:function(a, cb){return cb();}};
    /*return gulp.src(src).
        pipe(plumber({errorHandler: function(e){
            console.log(e);
        }})).
