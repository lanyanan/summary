...
/***/ function(module, exports, __webpack_require__) {
	...
	var _XymlEcharts = __webpack_require__(26);
	    ...
	    _createClass(App, [{
	        ...
	        value: function componentDidMount() {
	            ...
	            setTimeout(function () {
	                ...
	                _Actions.Actions.lastXyml(ajaxData);
	            }, 500);
	        }
	    }, {
	        key: 'render',
	        value: function render() {
	            var color = ['#FE4147', '#8DEABD'],
	                text = ['血氧', '脉率'];
	            //console.log(this.state)
	            ...
	            return React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    React.createElement(
	                        'div',
	                        { className: 'flex', style: { width: '90%', margin: "0 auto", marginTop: '-25px' } },
	                        React.createElement(
	                            'div',
	                ...
	                React.createElement(_XymlEcharts.XymlEcharts, { type: 'xyml', text: text, color: color })
	            );
	        }
	    }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var XymlEcharts = exports.XymlEcharts = function (_BaseComponent) {
	    _inherits(XymlEcharts, _BaseComponent);

	    function XymlEcharts(props) {
	        _classCallCheck(this, XymlEcharts);

	        var _this2 = _possibleConstructorReturn(this, (XymlEcharts.__proto__ || Object.getPrototypeOf(XymlEcharts)).call(this, props));
	    ...
	    _createClass(XymlEcharts, [{
	        ...
	        value: function render() {
	            ...
	            return React.createElement(
	                ...
	                React.createElement('section', { className: 'flex chart-container oxygenChart', id: '', ref: 'oxygenChart' }),
	                React.createElement('section', { className: 'flex chart-container pulseChart', id: 'chart', ref: 'pulseChart' }),
	                React.createElement(
	                    'section',
	        ...
	        value: function componentWillUpdate(nextProps, nextState) {
	            var _this = this,
	                ...
	                oMarkPointData = [],
	                pMarkPointData = [];
	            var oChartDom = ReactDOM.findDOMNode(this.refs.oxygenChart); // 血氧dom节点
	            var pChartDom = ReactDOM.findDOMNode(this.refs.pulseChart); // 脉率dom节点


	            //异常数据 标注处理
	            for (var i in oxygenYValue) {
	                if (oxygenYValue[i] < 90) {
	                    var obj = { name: '非常危险', value: oxygenYValue[i], xAxis: oxygenXValue[i], yAxis: oxygenYValue[i] };
	                    oMarkPointData.push(obj);
	                } else if (90 <= oxygenYValue[i] && 95 > oxygenYValue[i]) {
	                    var _obj = { name: '危险', value: oxygenYValue[i], xAxis: oxygenXValue[i], yAxis: oxygenYValue[i] };
	                    oMarkPointData.push(_obj);
	                }
	            }
	            for (var _i in pulseYValue) {
	                if (pulseYValue[_i] > 140 || pulseYValue[_i] < 40) {
	                    var _obj2 = { name: '非常危险', value: pulseYValue[_i], xAxis: oxygenXValue[_i], yAxis: pulseYValue[_i] };
	                    pMarkPointData.push(_obj2);
	                } else if (40 <= pulseYValue[_i] && 59 > pulseYValue[_i] || 101 <= pulseYValue[_i] && 140 >= pulseYValue[_i]) {
	                    var _obj3 = { name: '危险', value: pulseYValue[_i], xAxis: oxygenXValue[_i], yAxis: pulseYValue[_i] };
	                    pMarkPointData.push(_obj3);
	                }
	            }

	            // 基于准备好的dom，初始化echarts实例
	            var oChart = echarts.init(oChartDom),
